...
import { Loader, MoreVerticalIcon, PlusIcon, Trash2, User, UserCog, UserPlus } from "lucide-react";
...
const ProjectDetails = ({ project, member }: Props) => {
  ...
  const { isProjectOwner, isProjectMember, userProjectMembershipId } = useProjectAuthRole(project.id);
...
const handleDeleteProject = async () => {
  ...
  return (
    <div className="space-y-8">
      <ProjectHeader  project={project} progress={avancement}  onDelete={(isOpen: boolean, proj: Project) => openDeleteDialogProject(isOpen, project)}  />
      <Tabs defaultValue="toutes" className="w-full">
        <ProjectTabs />

        <TabsContent value="backlog">
          <TaskColumn title="Sprint" onTaskClick={handleTaskClick} isFullWidth />
        </TabsContent>
        ...
        <TabsContent value="calendrier">
          <TaskColumn title="À venir" onTaskClick={handleTaskClick} isFullWidth />
        </TabsContent>
        ...
        <TabsContent value="member">
          ...
          <div className="flex justify-end mb-4">
             {isProjectOwner&&(
                <Button variant="ghost" className="flex items-center gap-2" onClick={() => handleAddMemberProject(true)}>
            ...
            <Fragment key={m.membershipId}>
              <div className="flex items-center gap-3 border p-3 rounded-lg shadow-sm">
                ...
                <div className="flex flex-col">
                  ...
                  <p className="text-muted-foreground text-xs">{m.roleInProject || "Membre"}</p>
                </div>
              {isProjectOwner && m.id!=userProjectMembershipId && (
                <DropdownMenu>
                  ...
                  <DropdownMenuContent side="bottom" align="end">
                    ...
                    <DropdownMenuItem variant="destructive"  onClick={() => openDeleteDialog(m.id)}>
